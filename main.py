...
no_libration = read_vector_history_from_file('Pruebilla_no_lib.txt')
libration = read_vector_history_from_file('Pruebilla.txt')
diffs = compare_results(no_libration, libration, list(libration.keys()))
diffs_rsw = inertial2rsw(diffs, no_libration)

diffs = result2array(diffs)
diffs_rsw = result2array(diffs_rsw)

plt.figure()
plt.plot(diffs[:,0] / 86400.0, diffs[:,1], label = r'$X$')
plt.plot(diffs[:,0] / 86400.0, diffs[:,2], label = r'$Y$')
plt.plot(diffs[:,0] / 86400.0, diffs[:,3], label = r'$Z$')
# plt.plot(diffs[:,0] / 86400.0, norm_rows(diffs[:,1:4]), label = r'$r$')
plt.xlabel('Simulation time [days since J2000]')
plt.ylabel(r'$\Delta r$ [m]')
plt.grid()
plt.legend()
plt.title('Libration-generated differences in position')

plt.figure()
plt.plot(diffs_rsw[:,0] / 86400.0, diffs_rsw[:,1], label = r'$R$')
plt.plot(diffs_rsw[:,0] / 86400.0, diffs_rsw[:,2], label = r'$S$')
plt.plot(diffs_rsw[:,0] / 86400.0, diffs_rsw[:,3], label = r'$W$')
# plt.plot(diffs_rsw[:,0] / 86400.0, norm_rows(diffs_rsw[:,1:4]), label = r'$r$')
plt.xlabel('Simulation time [days since J2000]')
plt.ylabel(r'$\Delta r$ [m]')
plt.grid()
plt.legend()
