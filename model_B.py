...
In this script we will define model A2. It propagates the translational and rotational dynamics TOGETHER.
...
* Phobos' gravity field: user input from a file. Here, the one in Le Maistre (2019)
...
The tweaked rotational motion this model is of 2.28035245e-4 rad/s = 19.702245168 rad/day. The associated rotational
...
if verbose:
    if save_results: print('Simulating and saving full dynamics for all damping times...')
        ...
        print('WARNING: Results for each dissipation time overwrite the results for the preceding one. To keep them all, please save the results.')
tic = time()
for idx, current_damping_time in enumerate(dissipation_times):
    time_str = str(int(current_damping_time / 3600.0))
    ...
    current_propagator_settings = get_model_b_propagator_settings(bodies, simulation_time, initial_epoch, current_initial_state, dependent_variables_to_save)
...
if sum(checks) > 0:
    ...
    dependents_damped = current_simulator.dependent_variable_history
    epochs_array = np.array(list(dependents_damped.keys()))
    # epochs_array = np.array(list(states_undamped.keys()))
